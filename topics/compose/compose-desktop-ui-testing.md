...
If you are adding tests to an existing project, you may have to replace `sharedUI` in paths
and commands with the module name you are testing.

Create the test source set and add the necessary dependencies:

1. Create a directory for tests: `sharedUI/src/desktopTest/kotlin`.
2. In the `sharedUI/build.gradle.kts` file, add the following dependencies:
