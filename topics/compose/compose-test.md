...
to replace `sharedUI` in paths and commands with the module name you are testing.

Create a common test source set and add the necessary dependencies:

1. Create a directory for the common test source set: `sharedUI/src/commonTest/kotlin`.
2. In the `sharedUI/build.gradle.kts` file, add the following configuration:
...
In the `sharedUI/src/commonTest/kotlin` directory, create a file named `ExampleTest.kt` and copy the following code into it:
...
```shell
./gradlew :sharedUI:connectedAndroidTest
```
