...
1. Setup
    ...
    3. (Optional) Installing Xcode.
    4. Troubleshooting.
2. Creating a project.
    * Using the IDE wizard, **both** for IDEA and AS.
3. Running a project.
    * Using pregenerated run configurations.
      * Are all targets going to be available as run configurations?
4. What's next
   * Read how to implement a shared UI (first priority)
   * Read how to share only business logic along with a native UI (second priority)
   * In-depth KMP documentation
   * CMP framework documentation
   * Samples
     * Our sample page
     * GitHub topic(s)
   * Libraries
     * Talk about the fundamental/popular libraries which are already multiplatform.
       * coroutines, serialization etc.; okhttp, Ktor, coil, Room, Koin, SQLDelight  
         * TODO: ask dev advocates for a representative list.
     * Klibs.io
       * To find a specific library for your use case or simply to discover libraries already existing for KMP.
